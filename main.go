...
func notContains(slice []string, person string) bool {
...
func main() {
	...
	for i > 0 {
		...
		if (choice != President1.name) && (choice != President2.name) && (choice != President3.name) {
			log.Fatal("GET OUT OF HERE, YOU DON'T EVEN KNOW THE NAME OF THE CANDIDATES MTCHHEWWWW!!!!")
			return
		}
		...
		if voter.choice == President1.name {
			if notContains(President1.supporters, voter.name) && hashchecker(President1.people, voter.previous_hashid) {
			...
			if notContains(President2.supporters, voter.name) && hashchecker(President2.people, voter.previous_hashid) {
			...
			if notContains(President3.supporters, voter.name) && hashchecker(President3.people, voter.previous_hashid) {
